...
```{r}
mean(DS$hindfoot_length, na.rm = TRUE)
```

```{r}
mean(DS$hindfoot_length)
```

```{r}
aggregate(DS$hindfoot_length ~ year, DS, mean, na.action = na.omit)
```

```{r}
aggregate(DS$hindfoot_length ~ year, DS, mean)
```
...
```{r}
aggregate(DS$hindfoot_length ~ month, DS, mean, na.action = na.omit)
```

```{r}
aggregate(DS$hindfoot_length ~ month, DS, mean)
```
